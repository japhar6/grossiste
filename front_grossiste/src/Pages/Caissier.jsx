...
function Caisse() {
  ...
  // State for check date position - kept for future use
  const [datePositionnementCheque, setDatePositionnementCheque] = useState("");
  // State for notifications - kept for future use
  ...
  useEffect(() => {
    ...
    channel.bind("nouveau-comande", (data) => {
      ...
      setNotifications((prevNotifications) => {
        if (
          !prevNotifications.some((notif) => notif.message === data.message)
        ) {
  ...
  }, [fetchReferences, notificationSound]);
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await axios.get(
        `/api/commandes/reference/${referenceFacture}`
      );
  ...
  const validerPaiement = async () => {
    ...
    if (
      ...
      Swal.fire({
        ...
        text:
          modePaiement === "mobile money"
            ? "Veuillez entrer la référence de la transaction."
            : "Veuillez entrer la référence du bordereau.",
      });
      return false;
    }
    ...
    const data = {
      statut:
        commande.typeClient === "Commercial" ? "non payé" : "payé complet",
      modePaiement,
      totalPaye: commande.totalGeneral,
      referencePaiement,
      datePositionnementCheque:
        modePaiement === "cheque" ? datePositionnementCheque : null,
    ...
    try {
      ...
      // Reset commande and client data
      setCommande(null);
      setClient(null);
      setCommercial(null);
      setReferenceFacture("");
  ...
  const handleGenerateInvoice = async () => {
    try {
      const { value: typeFacture, isDismissed } = await Swal.fire({
        ...
        html: `
          <div style="text-align: left;">
            ...
            <br>
            <label>
              <input type="checkbox" name="typeFacture" value="normal" style="margin-right: 8px;" checked> 📜 Facture Normale
            </label>
            <br>
            <label>
        ...
        preConfirm: () => {
          const selectedTypes = Array.from(
            document.querySelectorAll('input[name="typeFacture"]:checked')
          ).map((input) => input.value);
          if (selectedTypes.length === 0) {
            Swal.showValidationMessage(
              "⚠️ Vous devez sélectionner au moins un type de facture !"
            );
            return false;
          }
      ...
      if (paiementValidationResult) {
        ...
        // Optimize invoice opening
        const openInvoices = () => {
          const windows = [];
          const params = queryParams.toString();
          
          // Open invoices in sequence with a small delay to prevent browser blocking
          const openWithDelay = (url, name, delay) => {
          ...
          if (typeFacture.includes("normal")) {
            actions.push(openWithDelay(`/facture?${params}`, "factureNormal", 100));
          }
          if (typeFacture.includes("remise")) {
            actions.push(openWithDelay(`/FactureRemise?${params}`, "factureRemise", 200));
          }
          if (typeFacture.includes("sans_prix")) {
            actions.push(openWithDelay(`/facturesansprix?${params}`, "facturesansprix", 300));
          }
          if (typeFacture.includes("8mm")) {
            actions.push(openWithDelay(`/8mm?${params}`, "8mm", 400));
          }

          return Promise.all(actions);
        };

        // Show loading while opening invoices
        Swal.fire({
          ...
          didOpen: () => {
            ...
            openInvoices().then(() => {
              ...
              Swal.fire('Erreur', 'Une erreur est survenue lors de l\'ouverture des factures', 'error');
            });
          }
        });
      }
      ...
      Swal.fire('Erreur', 'Une erreur est survenue lors de la génération des factures', 'error');
    }
  };
  const handleArgentDonneChange = (event) => {
    ...
    // Vérifier si la commande existe avant d'accéder à ses propriétés
    if (!commande) {
      setArgentRendu(0);
      return;
    }
          ...
          <div className="mini-stat p-3">
            ...
            <div className="commande-container d-flex justify-content-between">
              <div className="refcli ">
                <h6 className="gradient-text mb-3">
                  <i className="fa fa-user"></i> Référence de la commande
                </h6>
                <div className="form-group ">
                  ...
                  >
                    ...
                    {commande?.typeClient === "Client" ? (
                      <>
                        {commande?.clientId?.creerPar === "vendeur" &&
                        commande?.clientId?.nom ? (
                          <>
                  ...
                  {modePaiement === "cheque" && (
                    <div className="form-group">
                      <label htmlFor="datePositionnementCheque">
                        Date de positionement du cheque
                      </label>
                      <input
                        type="date"
                        ...
                        onChange={(e) =>
                          setDatePositionnementCheque(e.target.value)
                        }
                        required
                      />
                    </div>
                  ...
                  {modePaiement === "a credit" &&
                    (commande?.typeClient === "Commercial" ||
                      commande?.clientId?.creerPar === "admin") && (
                      <div className="form-group">
                        <label htmlFor="dateLimiteCredit">
                  ...
                  <button
                    className="btno btn-success "
                    disabled={loadingAction}
                    onClick={handleSearch}
                  >
                    {loadingAction ? (
                      <div
                        className="spinner-border style={{ width: '1rem !important', height: '1rem !important' }}"
                        role="status"
                      >
              ...
              <div className="produits p-3">
                <h6 className="alert alert-info" style={{ width: "100%" }}>
                  <i className="fa fa-box"></i> Détails du paiement{" "}
                </h6>
                <div
            ...
            {commande && (
              <div className="commandeX mt-4">
                <h6>
                  <i className="fa fa-receipt"></i> Récapitulatif de la Commande
                </h6>
                  ...
                  <tbody>
                    {commande.produits.map((produit, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          {produit.quantite} {produit.uniteChoisie}
                        </td>
                ...
                <div className="payment-container">
                  {commande.typeRemise !== null ? (
                    <h6 className="total mt-5 mb-4">
                      Total du commande après remise : {commande.totalGeneral}{" "}
                      Ariary
                    </h6>
                  ) : (
                    <h6 className="total mt-5 mb-4">
                  ...
                  />
                  <h5
                    className={`total-rendered mt-5 ${
                      argentRendu > 0 ? "positive" : "negative"
                    } `}
                  >
                    {argentRendu > 0
                      ? `Total rendu: ${argentRendu} Ariary`
                      : "Pas assez d'argent"}
                  </h5>
                </div>
                ...
                <button
                  ...
                  disabled={
                    commande?.clientId?.creerPar?.trim().toLowerCase() ===
                      "vendeur" &&
                    modePaiement.trim().toLowerCase() === "a credit"
                  }
